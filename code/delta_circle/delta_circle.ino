...
void loop() {
Serial.println("Enter Centre Coordinates");
while(Serial.available()==0){}
k=Serial.parseInt();
h=Serial.parseInt();//+0.15

Serial.println("Enter Radius");
while(Serial.available()==0){}  
r=Serial.parseInt();//+50     
z=-350;

if(drawCircle()==1){  
Moveto(0,0,-350);           // Point A
delay(1000);
...
bool checkworkvol(float x,float y){
...
Serial.println(String(i)+" "+String(x[i])+" "+String(y[i]));
if(checkworkvol(x[i],y[i])==0){
  return 0;
  break;
}
j=j+18;
